...
def scoring(prefix, lig_basename, pose_fpath, pock_g, model, out_fpath, work_dir):

    # parse poses
    _lig_basename = prefix + "_" + lig_basename
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ref_lig_fpath", type=str, default="ref_lig.sdf",
    ...
    work_dir = "temp_files" # The dir storing the temp files.
    ...
    lig_basename = os.path.basename(pose_fpath).split(".")[0]

    try:
        scoring(prefix, lig_basename, pose_fpath, pock_g, model, out_fpath, work_dir)
    except ValueError:
